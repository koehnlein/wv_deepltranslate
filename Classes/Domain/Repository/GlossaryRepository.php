...
use TimDreier\TdDeepltranslate\Service\Client\DeepLException;
...
class GlossaryRepository
{
    ...
    public function getGlossaryBySourceAndTargetForSync(
        ...
        if ($result === null) {
            ...
            $db = GeneralUtility::makeInstance(ConnectionPool::class)
                ->getConnectionForTable('tx_tddeepltranslate_glossary');
            $db->insert('tx_tddeepltranslate_glossary', $insert);
            $lastInsertId = $db->lastInsertId('tx_tddeepltranslate_glossary');
    ...
    {
        $db = GeneralUtility::makeInstance(ConnectionPool::class)
            ->getQueryBuilderForTable('tx_tddeepltranslate_glossary');
        $statement = $db
            ->select('uid', 'glossary_id')
            ->from('tx_tddeepltranslate_glossary')
            ->where(
    ...
    {
        $db = GeneralUtility::makeInstance(ConnectionPool::class)
            ->getQueryBuilderForTable('tx_tddeepltranslate_glossaryentry');
        $statement = $db
            ->select('uid', 'term')
            ->from('tx_tddeepltranslate_glossaryentry')
            ->where(
                $db->expr()->eq(
                    'pid',
    ...
    public function setGlossaryNotSyncOnPage(int $pageId): void
    {
        ...
        $queryBuilder->update('tx_tddeepltranslate_glossary')
